...
async function sendVerificationMail(userData, token) {
    ...
    mailTransporter.sendMail(mailDetails, function(err, data) {
        ...
        if(foundUser) {
            ...
            let link = `/authenticate/${newtoken}`
            let mailDetails = {
                from: 'sahildua899@gmail.com',
                to: foundUser.email,
                subject: 'Verify Your Email',
...
async function newUser(userData){
    ...
    const token = jwt.sign({phone:userData.phone,email:userData.email},'himynameissahilduafullstackwebdeveloper', {expiresIn:"2h"});
...
async function verifyUser(data) {
    ...
    const verify = jwt.verify(data.token,'himynameissahilduafullstackwebdeveloper')
...
async function loginUser(loginData) {
    ...
    return {statusCode:200, message:`Login Success Welcome ${foundUser.firstname} ${foundUser.lastname}`}
}
...
async function updateUserPass(newUserData) {
    const verifyUser = jwt.verify(newUserData.token,'himynameissahilduafullstackwebdeveloper' )
